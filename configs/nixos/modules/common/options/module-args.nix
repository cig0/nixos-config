...
}:
{
  /*
    ...
    `myArgs.packages.pkgsUnstable` to `_module.args.myArgs`.
    Before, it passed `pkgsUnstable` directly, which could confuse users seeing it as an input elsewhere.

    How to use it:

    - In a producer module, define the attr set to share:
      mySystem.myArgsContributions.fooBar = {
        foo = foo;
        bar = bar;
        quz = { zax = zax; };
      };
