...
*/
{
  ...
  inputs = {
    ...
    nixpkgs-unstable.url =
      "nixpkgs/nixos-unstable"; # The unstable release channel to allow for fresher packages
    ...
    lanzaboote = {
      inputs.nixpkgs.follows =
        "nixpkgs"; # Optional but recommended to limit the size of the system closure
  ...
  outputs = { auto-cpufreq, home-manager, lanzaboote, nix-flatpak, nix-index
    , nix-index-database, nix-ld, nix-snapd, nixos-cosmic, nixos-hardware
    , nixpkgs, nixpkgs-unstable, nixvim, plymouth-is-underrated, rust-overlay
    , self, sops-nix
    , # TODO_ https://github.com/users/cig0/projects/1/views/1?pane=issue&itemId=103254314&issue=cig0%7Cnixos-config%7C6
    yazi, ... }@inputs:
    let
      ...
      mkHost = hostname: hostConfig:
        let
          ...
          */
          ...
          platformLine =
            lib.findFirst (line: lib.hasInfix "nixpkgs.hostPlatform" line) ""
            lines;

          system = if platformLine != "" then
            lib.elemAt (lib.splitString ''"''
              (lib.elemAt (lib.splitString "=" platformLine) 1)) 1
          else
          ...
          modules = [
            ...
            */
...

            {
              ...
              # myArgs
              mySystem.myArgsContributions.system = { hostname = hostname; };
            }
